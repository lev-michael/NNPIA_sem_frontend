...
const MovieListItem = ({ movie, removeHandler, ratingUpdatedHandler }) => {
    ...
    return <div>
        {movie && <div className='movie-list-item flex flex--align-center'>
            ...
            <div className='padding-element--horizontal' style={{ height: "231px", width: "100%", position: "relative" }}>
                <div className="flex flex--align-center margin-element--top">
                    ...
                    <span className="margin-element--left-small margin-element--right">{movie.score ? `(${Math.round(movie.score * 10)}%)` : ""}</span>
                ...
                <div className="text text--label">Release date: {moment(movie.release_date).format("DD. MM. YYYY")}, {movie.runtime && <span>Runtime: {movie.runtime}min</span>}</div>
